// acpp -O3 --acpp-targets='hip:gfx906' amd_best_sycl.cpp -o amd_sycl_acpp
// clang++ -O3 -fsycl -fsycl-targets=amdgcn-amd-amdhsa -Xsycl-target-backend --offload-arch=gfx906 amd_best_sycl.cpp  -o amd_sycl_dpcpp
...
void test_mysgemm_v9(INT M, INT N, INT K, FLOAT alpha, FLOAT *A, FLOAT *B, FLOAT beta, FLOAT *C, sycl::queue& q_ct1);
...
#define vload(v1, addr) v1 = *((sycl::float4 *)(addr));
#define vstore(addr, v1) *((sycl::float4 *)(addr)) = v1;

void test_mysgemm_v1(INT M, INT N, INT K, FLOAT alpha, FLOAT *A, FLOAT *B, FLOAT beta, FLOAT *C, sycl::queue& q_ct1);


void mysgemm_v1(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C,
                sycl::nd_item<3> item_ct1){
    int lda = M, ldb = K, ldc = M;
    int tx = item_ct1.get_local_id(2), ty = item_ct1.get_local_id(1);
    int bx = item_ct1.get_group(2), by = item_ct1.get_group(1);
    // A,B 现在指向了对应的行列的地址；
    // 首先划定线程块对应的位置
    A = &A((bx<<5),0);
    B = &B(0,(by<<5));
...
void mysgemm_v9(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C,
                sycl::nd_item<3> item_ct1, float *sa9, float *sb9){
    ...
    sycl::float4 Av[param21], Bv[param22], Cv[param23], Cres[param23];
    ...
    for (int k_count = 0; k_count<K; k_count+=Kb){
        ...
        for (int cnt = 0; cnt < Mb*Kb/4/threadNums; cnt++) {
            vload(Av[0], &A(row_a + offsetA, col_a))
                ...
                // --> 需要扩多少bit
                ((sycl::float4 *)sa9)[(tx << param25) + cnt] = Av[0];
            offsetA += 4;
        }
        #pragma unroll
...
int main() {
    ...
    dA = (float *)sycl::malloc_device(sizeof(FLOAT) * max_size * max_size,
                                      q_ct1);
    dB = (float *)sycl::malloc_device(sizeof(FLOAT) * max_size * max_size,
                                      q_ct1);
    dC = (float *)sycl::malloc_device(sizeof(FLOAT) * max_size * max_size,
                                      q_ct1);
    dC_ref = (float *)sycl::malloc_device(sizeof(FLOAT) * max_size * max_size,
                                          q_ct1);
    q_ct1.memcpy(dA, A, sizeof(FLOAT) * max_size * max_size).wait();
    q_ct1.memcpy(dB, B, sizeof(FLOAT) * max_size * max_size).wait();
    q_ct1.memcpy(dC, C, sizeof(FLOAT) * max_size * max_size).wait();
    q_ct1.memcpy(dC_ref, C_ref, sizeof(FLOAT) * max_size * max_size).wait();
    ...
    for (int i_count=0;i_count<upper_limit;i_count++){
		...
		// 调用我们的 kernel 情况
		test_mysgemm_v9(m, n, k, alpha, dA, dB, beta, dC, q_ct1);
                q_ct1.wait_and_throw();

        // 调用 naive 版本进行验证
        test_mysgemm_v1(m, n, k, alpha, dA, dB, beta, dC_ref, q_ct1);
                q_ct1.wait_and_throw();

                // 把我们的kernel计算的结果复制到 host
                q_ct1.memcpy(C, dC, sizeof(FLOAT) * m * n).wait();
        q_ct1.memcpy(C_ref, dC_ref, sizeof(FLOAT) * m * n).wait();
                q_ct1.wait_and_throw();
    ...
    sycl::free(dA, q_ct1); sycl::free(dB, q_ct1); sycl::free(dC, q_ct1);
        sycl::free(dC_ref, q_ct1);
    q_ct1.wait_and_throw();
    return 0;
}
...
void test_mysgemm_v9(INT M, INT N, INT K, FLOAT alpha, FLOAT *A, FLOAT *B,
                     FLOAT beta, FLOAT *C, sycl::queue &q_ct1) {
    //dpct::device_ext &dev_ct1 = dpct::get_current_device();
    q_ct1.wait_and_throw();
    sycl::range<3> gridDim1(1, CEIL_DIV(N, 128), CEIL_DIV(M, 128));
        sycl::range<3> blockDim1(1,1,512);
        /*
    ...
    // TODO: 此处的调用十分的棘手，我们应该如何做替换？
    q_ct1.submit([&](sycl::handler &cgh) {
        sycl::local_accessor<float, 1> sa9_acc_ct1(sycl::range<1>(2048), cgh);
        sycl::local_accessor<float, 1> sb9_acc_ct1(sycl::range<1>(2048), cgh);

        cgh.parallel_for(
            sycl::nd_range<3>(gridDim1 * blockDim1, blockDim1),
            [=](sycl::nd_item<3> item_ct1) {
                mysgemm_v9
	<128,16,128,16,64,4,1,8,7,3,3,3,512,9,2,3,2,4,4,2,6,3,5,31,2,7,7,2048,2048,1,2,8,2,0>
	(M,N,K,alpha,A,B,beta,C,item_ct1,sa9_acc_ct1.get_pointer(),sb9_acc_ct1.get_pointer());
            });
    });
    q_ct1.wait_and_throw();
}
...
void test_mysgemm_v1(INT M, INT N, INT K, FLOAT alpha, FLOAT *A, FLOAT *B,
    ...
    //dpct::device_ext &dev_ct1 = dpct::get_current_device();
    q_ct1.wait_and_throw();
    sycl::range<3> blockDim2(1, 32, 32);
    sycl::range<3> gridDim2(1, CEIL_DIV(N, 32), CEIL_DIV(M, 32));
    /*
    ...
    */
    q_ct1.parallel_for(
        ...
        [=](sycl::nd_item<3> item_ct1) {
            mysgemm_v1(M, N, K, alpha, A, B, beta, C, item_ct1);
        });
    q_ct1.wait_and_throw();
}
